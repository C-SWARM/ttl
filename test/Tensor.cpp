...
TEST(Tensor, CtorWiden) {
  ...
  EXPECT_EQ(A.get(1), float(E));
  EXPECT_EQ(A.get(2), PI);
}
...
TEST(Tensor, MoveCtorWiden) {
  ...
  EXPECT_EQ(A.get(0), 0.0);
  EXPECT_EQ(A.get(1), 1.0);
  EXPECT_EQ(A.get(2), 2.0);
  EXPECT_EQ(A.get(3), 3.0);
}
...
TEST(Tensor, FillWiden) {
  ttl::Tensor<2, 2, double> A;
  A.fill(2);
  EXPECT_EQ(A.get(0), 2.0);
  EXPECT_EQ(A.get(1), 2.0);
  EXPECT_EQ(A.get(2), 2.0);
  EXPECT_EQ(A.get(3), 2.0);
}
...
TEST(ExternalTensor, CopyCtor) {
  ...
  ttl::Tensor<2, 2, int*> A(a);
  ttl::Tensor<2, 2, int*> B = A;
  EXPECT_EQ(B.get(0), a[0]);
  EXPECT_EQ(B.get(1), a[1]);
  EXPECT_EQ(B.get(2), a[2]);
  EXPECT_EQ(B.get(3), a[3]);
}
...
TEST(ExternalTensor, CopyCtorConst) {
  ...
  EXPECT_EQ(D.get(0), a[0]);
  EXPECT_EQ(D.get(1), a[1]);
  EXPECT_EQ(D.get(2), a[2]);
  EXPECT_EQ(D.get(3), a[3]);
}
